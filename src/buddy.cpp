...
// Cat (Bauble) is index 0 — that's the boot default when no species has
// been saved to NVS yet, and it's where the BtnB cycle wraps back to.
static const Species* SPECIES_TABLE[] = {
  &CAT_SPECIES, &CAPYBARA_SPECIES, &DUCK_SPECIES, &GOOSE_SPECIES,
  &BLOB_SPECIES, &DRAGON_SPECIES, &OCTOPUS_SPECIES, &OWL_SPECIES,
